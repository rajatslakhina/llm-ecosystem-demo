...
struct EcosystemDemo {
    static func main() async {
        ...
        await runAuthorityScenarios(meter: meter)
        ...
        await runAnswerabilityScenario(meter: meter)
        ...
        print("Total metered cost across all seventy-three scenarios: $\(await meter.totalCost())")
    }
    ...
    private static func printBanner() {
            ...
            print(
                "ProviderGatewayKit (routing) + StructuredOutputKit (decoding) + TokenMeterKit (cost) + "
                    ...
                    + "SourceConflictKit (and the question every one of those asks too late: do the "
                    + "retrieved passages agree with each other - audited before the turn is sent, "
                    + "corroboration counted in documents rather than chunks)\n"
            )
    }
